...
def str_to_timestamp(raw):
    ...
    '''
    if 'T' in raw:
        s = raw.split('T')[0] + ' ' + raw.split('T')[1].split(':')[0] + ':' + raw.split('T')[1].split(':')[1]
...
if __name__ == '__main__':
    ...
    print('Словарь рассчитан')

    file_list = os.listdir(PATH_TO_DIRECTORY_DEALS)
    print('Список файлов для рассчета', file_list)
    ...
    for file in file_list:
        ...
        short_df.to_csv(PATH_TO_DIRECTORY_RESULT + '\\' +'v0_Deals_USD_short_' + file, index=False)
